...
def push_main_branch(dry_run: bool, stamp: str):
    ...
    if commit.returncode != 0:
        ...
        return False
    if dry_run:
        print("[main] 试运行，不推送远程")
        return True
    print(f"[main] 推送 origin/{MAIN_BRANCH}")
...
def deploy_gh_pages(dry_run: bool):
    ...
    if has_branch:
        ...
        print(f"\n[Git] 创建孤儿分支 {GH_BRANCH}")
        run_git(["worktree", "add", "--orphan", "-b", GH_BRANCH, str(WORKTREE_DIR)])
    if WORKTREE.exists() and (WORKTREE_DIR / ".git").exists():
    ...
    print("[Git] 复制 index / html 至gh-pages")
    run_git(["add", "-A"], cwd=WORKTREE_DIR)
    stamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    commit_msg = f"更新索引 {stamp}\n自动生成"
    commit = run_git(["commit", "-m", commit_msg], cwd=WORKTREE_DIR, capture=True)
    if commit.returncode == 0:
        print("[Git] gh-pages 提交成功")
    else:
        print("[Git] gh-pages 无变更")
    if dry_run:
        print(f"\n[试运行] 临时目录 {WORKTREE_DIR}，不推送gh-pages")
        return True, stamp
    print(f"[Git] 推送 {GH_BRANCH}")
    push = run_git(["push", "origin", GH_BRANCH], cwd=WORKTREE_DIR, capture=True)
    if push.returncode != 0:
        print(f"[Git] gh-pages推送失败 {push.stderr}")
        return False, stamp
    print("[Git] gh-pages推送完成")
    clean_old_worktree()
    return True, stamp

def main():
    import argparse
    parser = argparse.ArgumentParser(description="Thon Market 自动构建+推送main+gh-pages")
    parser.add_argument("--dry-run", action="store_true", help="试运行，不推送远程")
    parser.add_argument("--skip-git", action="store_true", help="仅生成文件，跳过git全部操作")
