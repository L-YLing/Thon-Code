...
class ChangelogWindow:
    ...
    def _create_left_panel(self, parent):
        ...
        ctk.CTkLabel(header_frame, text=self._get_text("changelog_version"), font=("Microsoft YaHei", 12, "bold")).pack(side="left", padx=10)
    ...
    def _create_right_panel(self, parent):
        ...
        ctk.CTkLabel(version_frame, text=self._get_text("changelog_version"), width=80).pack(side="left", padx=(10, 5))
        self.version_entry = ctk.CTkEntry(version_frame, width=200, placeholder_text=self._get_text("changelog_enter_version"))
        self.version_entry.pack(side="left", padx=5)
        
        # Date field
        date_frame = ctk.CTkFrame(form_frame, fg_color="transparent")
        date_frame.pack(fill="x", pady=5)
        ctk.CTkLabel(date_frame, text=self._get_text("changelog_date"), width=80).pack(side="left", padx=(10, 5))
        self.date_entry = ctk.CTkEntry(date_frame, width=200, placeholder_text=self._get_text("changelog_enter_date"))
        self.date_entry.pack(side="left", padx=5)
        self.date_entry.insert(0, datetime.now().strftime("%Y-%m-%d"))
        
        # Sections management
        sections_frame = ctk.CTkFrame(form_frame)
        sections_frame.pack(fill="both", expand=True, pady=(5, 5))
        
        # Section tabs (listbox)
        tab_frame = ctk.CTkFrame(sections_frame, fg_color="transparent")
        tab_frame.pack(fill="x", pady=2)
        
        ctk.CTkLabel(tab_frame, text="Sections:", font=("Microsoft YaHei", 11, "bold")).pack(side="left", padx=5)
        
        ctk.CTkButton(tab_frame, text="+ Add Section", command=self._add_section, width=100).pack(side="right", padx=2)
        ctk.CTkButton(tab_frame, text="- Remove", command=self._remove_section, width=80, fg_color="#8a2d2d").pack(side="right", padx=2)
        ...
        items_frame = ctk.CTkFrame(sections_frame)
        items_frame.pack(fill="both", expand=True, pady=(5, 0))
        
        ctk.CTkLabel(items_frame, text="Items:", font=("Microsoft YaHei", 11, "bold")).pack(anchor="w", padx=10)
        ...
        self.item_entry = ctk.CTkEntry(item_input_frame, width=400, placeholder_text="Enter item, press Enter to add...")
        self.item_entry.pack(side="left", padx=2)
        self.item_entry.bind("<Return>", self._add_item)
        
        ctk.CTkButton(item_input_frame, text="Add Item", command=self._add_item, width=80).pack(side="left", padx=2)
        ctk.CTkButton(item_input_frame, text="Remove Selected", command=self._remove_item, width=100, fg_color="#8a2d2d").pack(side="left", padx=2)
        
        # Save/Cancel buttons
        save_frame = ctk.CTkFrame(parent, fg_color="transparent")
        save_frame.pack(fill="x", pady=5)
        
        ctk.CTkButton(save_frame, text=self._get_text("changelog_save"), command=self._save_entry, width=120, fg_color="#2d7d2d").pack(side="right", padx=10)
        ctk.CTkButton(save_frame, text=self._get_text("changelog_cancel"), command=self._clear_form, width=100, fg_color="#555555").pack(side="right", padx=5)
    ...
    def _load_changelog(self):
        ...
        self._update_status(f"Loaded {len(self.changelog.entries)} entries")
    ...
    def _add_section(self):
        """Add a new section"""
        dialog = ctk.CTkInputDialog(text="Enter section name (e.g., Added, Fixed, Changed):", title="Add Section")
        ...
        if section_name in self.section_items:
            messagebox.showinfo("Info", f"Section '{section_name}' already exists")
            return
    ...
    def _remove_section(self):
        ...
        if not selection:
            messagebox.showinfo("Info", "Select a section to remove")
            return
        
        section_name = self.section_tabs.get(selection[0])
        if messagebox.askyesno("Confirm", f"Remove section '{section_name}' and all its items?"):
    ...
    def _save_entry(self):
        ...
        if not version:
            messagebox.showerror("Error", self._get_text("changelog_invalid_version"))
            return
        if not date:
            messagebox.showerror("Error", self._get_text("changelog_invalid_date"))
            return
        ...
        if not sections:
            messagebox.showerror("Error", self._get_text("changelog_no_changes"))
            return
        
        existing = self.changelog.get_entry(version)
        if existing and (not self.current_entry or self.current_entry.version != version):
            if not messagebox.askyesno("Confirm", self._get_text("changelog_version_exists").format(version=version) + "\nOverwrite?"):
                return
        ...
        if self.changelog.add_entry(version, date, sections):
            ...
            messagebox.showerror("Error", "Failed to save changelog")
    
    def _remove_entry(self):
        """Remove selected entry"""
        selection = self.version_listbox.curselection()
        if not selection:
            messagebox.showinfo("Info", "Select a version to remove")
            return
        version = self.version_listbox.get(selection[0])
        if version == self._get_text("changelog_no_entries"):
            return
        if messagebox.askyesno("Confirm", self._get_text("changelog_confirm_delete").format(version=version)):
        ...
        if file_path.endswith('.json'):
            ...
            try:
                ...
                if entries:
                    ...
                    messagebox.showerror("Error", "No valid entries found in file")
            except Exception as e:
    ...
    def _preview_changelog(self):
        ...
        if not self.changelog.entries:
            messagebox.showinfo("Info", self._get_text("changelog_no_entries"))
            return
        ...
        text_widget = ctk.CTkTextbox(text_frame, font=("Consolas", 11))
