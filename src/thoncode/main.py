...
class MainWindow:
    ...
    def _build_menu(self):
        ...
        menubar.add_cascade(label=self.langs_cfg.main_root_project, menu=project_menu)
        project_menu.add_command(label=self.langs_cfg.main_root_project_cfg, command=self.show_project_config)

        file_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label=self.langs_cfg.main_root_file, menu=file_menu)
        file_menu.add_command(label=self.langs_cfg.main_root_new, accelerator="Ctrl+N", command=self.new_file)
        file_menu.add_command(label=self.langs_cfg.main_root_open, accelerator="Ctrl+O", command=self.open_file)
        file_menu.add_command(label=self.langs_cfg.main_root_save, accelerator="Ctrl+S", command=self.save_file)
        file_menu.add_command(label=self.langs_cfg.main_root_save_as, accelerator="Ctrl+Shift+S", command=self.save_as)
        file_menu.add_separator()
        file_menu.add_command(label=self.langs_cfg.main_root_exit, accelerator="Ctrl+Q", command=self.exit_app)

        edit_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label=self.langs_cfg.main_root_edit, menu=edit_menu)
