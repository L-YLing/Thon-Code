...
# 简介
**这是一个为开源而生的IDE，由Python驱动，并且足够轻量，您甚至可以在十年前的主机上流畅运行（如果您想体验Zed，又受困于电脑配置，那么您可以试试这款IDE）。经过测试，打开一些中小型项目，ThonCode的启动速度与VSCode不差多少(约慢于`VSCode` 0.413s，[点击查看测试信息](#测试))。我们也在积极寻找解决运行时资源占用高、性能弱、启动速度慢等问题。**
...
> **内容已过时：在最新版本中，基于懒加载的优化，大型项目`Thon Code`已可以做到秒开(因为仅仅加载了第一层目录结构，仅在用户展开文件夹时才继续加载下一层内容，当然大文件仍然无法秒开，以及搜索功能目前因为懒加载的原因导致无法正常使用，但是我们已经有了优化思路预计0.0.13版本将会上线)**
