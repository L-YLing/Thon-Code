...
# 注册回调
tree.set_on_node_open(lambda node: print(f"打开: {node.path}"))
tree.set_on_node_select(lambda node: print(f"选中: {node.name}"))
...
```python
# 懒加载：子节点仅在展开时加载
tree.set_project_root("/project")  # 仅加载根目录
tree.expand_node(some_folder)       # 此时加载子节点
```
